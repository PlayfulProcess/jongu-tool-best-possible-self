-- =====================================================
-- IMPROVED JOURNAL TEMPLATES SCHEMA
-- Based on Supabase copilot feedback
-- =====================================================
...
BEGIN
  ...
  DROP POLICY IF EXISTS "Only playfulprocess.com can set AI prompts" ON public.journal_templates;
END$$;
...
CREATE POLICY "Authenticated users can create templates"
  ...
  WITH CHECK (
    ...
    (
      ai_prompt IS NULL
      OR
      (SELECT (auth.jwt() -> 'email')::text ILIKE '%@playfulprocess.com')
    )
  );
...
STABLE
AS $$
  SELECT (auth.jwt() -> 'email')::text ILIKE '%@' || domain;
$$;
